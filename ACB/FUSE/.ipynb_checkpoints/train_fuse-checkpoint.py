import torch
import torch.nn as nn
import torch.optim as optim
from torch.autograd import Variable
import numpy as np
from timeit import default_timer
from torch.optim.lr_scheduler import ReduceLROnPlateau
from FUSE import FUSE
import yaml

import sys
sys.path.append('../')
from utilities import *
from dataset import LoadTurbTowers


seed = 0
torch.manual_seed(seed)
l1_loss = torch.nn.L1Loss()
    
def train(model, train_loader, val_loader, device, config, input_mins, input_maxs, p_min, p_max):
    # Set optimizer and scheduler hyperparameters
    lr = config['parameters']['learning_rate']['value']
    batch_size = config['parameters']['batch_size']['value']
    wd = config['parameters']['weight_decay']['value']
    factor = config['parameters']['factor']['value']
    patience = config['parameters']['patience']['value']

    # Create the optimizer and scheduler
    optimizer = optim.Adam(model.parameters(), lr=lr, weight_decay=wd)
    scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=factor, patience=patience, verbose=True)

    # Define some numbers for training and calculating the average errors
    epochs = config['parameters']['epochs']['value']
    n_train = config['parameters']['n_train']['value']
    n_test = config['parameters']['n_test']['value']

    # Create tensors to store the scores/errors
    train_loss_in = torch.zeros(int(np.ceil(n_train/batch_size)))
    train_loss_out = torch.zeros(int(np.ceil(n_train/batch_size)))
    test_loss_in = torch.zeros(n_test)
    test_loss_out = torch.zeros(n_test)

    # Does the problem require padding? Set the value here
    pad = config['parameters']['pad']['value']

    # If we are saving the model, after what error do we start saving? define it here
    save_model = config['parameters']['save_model']['value']
    best_model = 100 # some large value ensures we start saving immediately

    
    train_loss = 0
    for epoch in range(epochs):
        model.train()
        t1 = default_timer()
        iter = 0
        for theta, x, y in train_loader:
            # Pad input if required
            if pad >0:
                x = extrapolate_zeros(x, pad)

            # Send data to GPU
            x = x.to(device)
            y = y.to(device)
            theta = theta.to(device)


            # Calculate the loss of the measure matching
            in_loss = model.param_loss(theta, x)
            optimizer.zero_grad()
            in_loss.backward()
            optimizer.step()

            # Compute prediction on output space, remove padding
            pred = model.y_prediction(theta, n_points = x.shape[-1])
            if pad > 0:
                pred = pred[:,:,pad:-pad]
                
            # Compute the loss 
            out_loss = l1_loss(pred, y) + 100 * h1_loss(pred, y)
            optimizer.zero_grad()
            out_loss.backward()
            optimizer.step()
            
            # training losses
            train_loss_in[iter] = in_loss.item()
            train_loss_out[iter] = out_loss.item()

            iter += 1

        t2 = default_timer()

        # Print statistics of this epoch
        print(f'epoch: {epoch} \t time: {t2-t1}')
        print(f'median training score: {torch.median(train_loss_in).item():.4f} average score: {torch.mean(train_loss_in).item():.4f} +/- {torch.std(train_loss_in).item():.4f} ')
        print(f'median training error: {torch.median(train_loss_out).item():.4f} average error: {torch.mean(train_loss_out).item():.4f} +/- {torch.std(train_loss_out).item():.4f} ')

        # Validation
        with torch.no_grad():   
            model.eval()
            test_loss = 0
            iter = 0
            for theta, x, y in val_loader:
                if pad > 0:
                    x = extrapolate_zeros(x, pad)

                x = x.to(device)
                y = y.to(device)
                theta = theta.to(device)

                # For validation, only consider each metric separately. Predictions are unified at test time. 
                in_loss = model.param_loss(theta, x)
                pred = model.y_prediction(theta, n_points=x.shape[-1])

                if pad > 0:
                    pred = pred[:,:,pad:-pad]
               
                out_error = relative_l1_error(pred, y)
                
                test_loss_in[iter] = in_loss.item()
                test_loss_out[iter] = out_error.item()
                iter += 1

            scheduler.step(torch.median(test_loss_in))

            
            print(f'median validation score: {torch.median(test_loss_in).item():.4f} average score: {torch.mean(test_loss_in).item():.4f} +/- {torch.std(test_loss_in).item():.4f} ')
            print(f'median validation error: {torch.median(test_loss_out).item():.4f} average error: {torch.mean(test_loss_out).item():.4f}% +/- {torch.std(test_loss_out).item():.4f}% ')
            print('\n')
            

        
        # Save the best performing model over the validation set
        test_error = torch.mean(test_loss_in).item() + torch.mean(test_loss_out).item()
        if save_model:
            if test_error < best_model:
                print('******************')
                print(f'Saving model with error: {test_error:.4f}')
                torch.save(model, f'FUSE_TurbTowers_{n_train}.pt')
                best_model = test_error
            

def main():
    with open("config.yaml", "r") as f:
        config = yaml.load(f, Loader=yaml.Loader)

    # Parameters regarding the datasets
    n_train = config['parameters']['n_train']['value']
    n_test = config['parameters']['n_test']['value']
    n_val = config['parameters']['n_val']['value']
    batch_size = config['parameters']['batch_size']['value']
    data_path = config['parameters']['data_path']
    
    # Load the data from the configs
    loader = LoadTurbTowers()
    train_loader, val_loader, test_loader, input_mins, input_maxs, p_min, p_max, n_points = loader.get_dataloaders(n_train, n_val, n_test, batch_size, data_path)
    
    # Parameters regarding the model size
    n_parameters = p_min.shape[0]    
    channels_in = input_mins.shape[1]
    channels_target = channels_in
    fmpe_layers = config['parameters']['fmpe_layers']['value']
    fmpe_width = config['parameters']['fmpe_width']['value']
    fno_in_layers = config['parameters']['fno_in_layers']['value']
    fno_out_layers = config['parameters']['fno_out_layers']['value']
    fno_modes = config['parameters']['fno_modes']['value']
    fno_width = config['parameters']['fno_width']['value']

    # Check computing resources
    if torch.cuda.is_available():
        device='cuda:0'
    else:
        device='cpu'

    # Initialize the model, send to GPU
    model = FUSE(fmpe_layers, fmpe_width, n_parameters, fno_in_layers, fno_out_layers, fno_width, fno_modes, channels_in, channels_target)
    print(count_params(model))
    model.to(device)  

    # Finally ready to train the model!
    train(model, train_loader, val_loader, device, config, input_mins, input_maxs, p_min, p_max)
    
    
if __name__ == "__main__":
    main()
    
    
