...
def continuous_errors(pred, true):
    l1_errors = torch.zeros((100,))
    l2_errors = torch.zeros((100,))
    ...
    pred = pred.reshape(100, 2, 10, 181)
    true = true.reshape(100, 2, 10, 181)
...
def continuous_crps(pred, true):
    ...
    for sample in range(true.shape[0]):
        crps[sample] = crps_per_continuous(pred[sample], true[sample])
        
    print(f"continuous CRPS: {np.mean(crps).item()} +/- {np.std(crps).item()}")

def discrete_errors(pred, true):
    crps_scores = np.zeros((100,))
...
def main():
    ...
    file_path = "../../../_Data/cVANO_results/ACB/"
        # Load the compressed npz file
    data = np.load(file_path+'turb_towers_results_decoding_latent_vectors_of_true_parameters_outofdistribution.npz')
    ...
    file_path = "/cluster/work/math/camlab-data/Flowperator/TurbTowers/"
        # Load the compressed npz file
    data = np.load(file_path+'turb_towers_results_using_infered_parameters_outofdistribution.npz')
